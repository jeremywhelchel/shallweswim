...
- `LocalHarmonicTidesFeed` supports locations where a suitable external tide
  ...
  and locally produces the same short high/low prediction window as CO-OPS.
- The tide dataframe's `prediction` column is in feet. NOAA CO-OPS already
  provides feet; local harmonic models may be meter-native and must convert to
  feet before publishing data to the rest of the app.
