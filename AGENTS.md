...
10. **Install Playwright Chromium only when needed** - Python Playwright and frontend Playwright are pinned to the same version and share the same browser cache. Do not run browser installation as a routine checklist step. If `--run-browser` tests or `corepack pnpm@10.18.3 --dir frontend test:e2e` report missing Chromium, run `uv run playwright install chromium` and retry. This is project-local test environment preparation; do not run `playwright install-deps` unless system packages are explicitly needed and approved.
