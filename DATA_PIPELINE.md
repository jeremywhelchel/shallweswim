...
Every reading the conditions endpoint returns says how current it is,
decided by the server so that every client shows the same thing, and the
page changes how it presents a reading as it ages. Carry-forward keeps a
feed's last frame in the bundle without limit, so without this a reading
...
always present: an old reading is still the last known reading. The age is
measured from the observation time, not from the fetch, so a source that
keeps answering with the same last reading ages like one that stopped
answering. The thresholds are two named constants next to the feed
intervals, the same for both observation feeds:

| Feed | `fresh` until | `stale` until | then `old` |
| --- | --- | --- | --- |
| live temperature | 2 hours | 24 hours | |
| observed currents | 2 hours | 24 hours | |

The fresh threshold is deliberately looser than the fetch interval: a
...
absent. The tide state estimate and the legacy chart, both derived from the
tide predictions, stop at that window too.

Why: observations degrade with age and predictions do not, which is why
they get different rules; the thresholds are two constants in one place
rather than in the frontend; and the decision is the server's, one field
per condition, so the page and every embed do no clock arithmetic of their
own. The job's stale verdict is a different question, whether the pipeline
is delivering, and keeps measuring the fetch age.
